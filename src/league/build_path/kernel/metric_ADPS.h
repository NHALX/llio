...
opencl_allocinfo metric_ADPS__allocnfo__(opencl_function *, size_t items_n);
result_t metric_ADPS(__global ideal_t *linext, DB, llf_criteria *cfg, lattice_info *info, count_t nth_extension);
#else
__kernel void metric_ADPS(ulong_t linext_offset, lattice_info info, __global ideal_t *linext, DB, llf_criteria cfg, __local ideal_t *pasv_scratch, __local result_t* scratch, __global result_t* result);
#endif
#endif
