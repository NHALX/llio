...
c_result_t *
gpu_LE(struct gpu_context *ctx, struct ideal_lattice *g, c_itemid_t *idmap, size_t idmap_len, cl_float cfg_input[CFG_SIZE], size_t *result_n)
{
	cl_int error;
	cl_mem imem, cmem, amem, db, passives, cfg, idmapmem, outmem;
	c_result_t *results;
	...
	*result_n = global_size / local_size;
	...
	idmapmem = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY, idsiz, NULL, &error);
	...
	NOFAIL(clSetKernelArg(ctx->kernel_LE, 3, sizeof(idmapmem), &idmapmem))
	...
	NOFAIL(clEnqueueWriteBuffer(ctx->queue, idmapmem, CL_FALSE, 0, idsiz,                     idmap, 0, NULL, NULL))
	...
	NOFAIL(clEnqueueWriteBuffer(ctx->queue, amem,     CL_FALSE, 0, asiz,                      g->neighbors, 0, NULL, NULL))
